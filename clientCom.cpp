...
bool clientCom::set_IP_and_connect(string sIP,int port)
{
    ...
    int err=connect(m_SocServer, (SOCKADDR*) &target, sizeof(target));//will always give error if non-blocking
    /*if(err!=0)
    {
        cout<<err<<endl;
        cout<<WSAGetLastError()<<endl;
    }*/
    cout<<"Connecting to "<<sIP<<":"<<port<<endl;
...
bool clientCom::send_data(int* data_array)
{
    int val=send(m_SocServer,(char*)data_array,sizeof(int)*data_array[0],0);

    if(val!=int(data_array[0]*sizeof(int))) return false;//sent wrong amount
...
bool clientCom::recv_data(int* data_array)
{
    int val=recv(m_SocServer,(char*)data_array,sizeof(int)*256,0);

    if(val!=int(data_array[0]*sizeof(int))) return false;//received wrong amount
