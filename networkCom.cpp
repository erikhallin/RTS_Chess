...
bool networkCom::init(string status)
{
    ...
    if(status=="server")
    {
        ...
        if(!m_ServerCom.init())
        {
            cout<<"ERROR: Could not init Server\n";
        }
        else m_ready=true;
    }
    ...
    if(status=="client")
    {
        ...
        if(!m_ClientCom.init())
        {
            cout<<"ERROR: Could not init Client\n";
        }
        else m_ready=true;
    }
...
bool networkCom::send_data(int* data_array)
{
    if(m_net_status==net_server)
    {
        m_ServerCom.send_data(data_array);
...
bool networkCom::recv_data(int* data_array)
{
    if(m_net_status==net_server)
    {
        m_ServerCom.recv_data(data_array);
        return true;
    }
    if(m_net_status==net_client)
    {
        m_ClientCom.recv_data(data_array);
        return true;
    }
