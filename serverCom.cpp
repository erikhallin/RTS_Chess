...
bool serverCom::send_data(int* data_array)//send to all clients
{
    bool error_flag=false;
    for(int i=0;i<(int)m_vSocClients.size();i++)
    {
        int val=send(m_vSocClients[i],(char*)data_array,sizeof(int)*data_array[0],0);

        if(val!=int(data_array[0]*sizeof(int))) error_flag=true;//received wrong amount
    }
...
bool serverCom::recv_data(int* data_array)
{
    //assume only one client
    if(m_vSocClients.size()>1)
    {
        cout<<"ERROR: NETCOM: More Clients than expected\n";
        return false;
    }
    else if(m_vSocClients.size()==0)
    {
        cout<<"ERROR: NETCOM: Too few Clients than expected\n";
        return false;
    }

    int val=recv(m_vSocClients[0],(char*)data_array,sizeof(int)*256,0);
